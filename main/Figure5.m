...
figure();
%alpha
OBCI = alphaPOBCIAllSubjects;
BP = alphaPBPAllSubjects;
[p,h] = ranksum(OBCI',BP','tail', 'both');
%[h,p] = ttest(OBCI',BP','tail', 'both');
[seOBCI,bs] = getSEMedian(OBCI');
[seBP,bs] = getSEMedian(BP');
DeltaBandPowers(1,:) = {median(OBCI'),seOBCI,median(BP'),seBP,p};
fig1 = subplot(1,3,1);
boxplot([OBCI', BP'], 'Labels',{'OBCI','BP'});
title('Alpha')
% slow Gamma
OBCI = slowGammaPOBCIAllSubjects;
BP = slowGammaPBPAllSubjects;
%[h,p] = ttest(OBCI',BP','tail', 'both');
[p,h] = ranksum(OBCI',BP','tail', 'both');
[seOBCI,bs] = getSEMedian(OBCI');
[seBP,bs] = getSEMedian(BP');
DeltaBandPowers(2,:) = {median(OBCI'),seOBCI,median(BP'),seBP,p};
fig2 = subplot(1,3,2);
boxplot([OBCI', BP'], 'Labels',{'OBCI','BP'});
title('Slow Gamma')
% fast Gamma
OBCI = fastGammaPOBCIAllSubjects;
BP = fastGammaPBPAllSubjects;
%[h,p] = ttest((OBCI'- BP'))
[p,h] = ranksum(OBCI',BP','tail', 'both');
[seOBCI,bs] = getSEMedian(OBCI');
[seBP,bs] = getSEMedian(BP');
DeltaBandPowers(3,:) = {median(OBCI'),seOBCI,median(BP'),seBP,p};
fig3 = subplot(1,3,3);
boxplot([OBCI', BP'], 'Labels',{'OBCI','BP'});
title('Fast Gamma')
DeltaBandPowers.Properties.VariableNames = {'OBCImedian','OBCISE','BPmedian','BPSE','p-val'};
