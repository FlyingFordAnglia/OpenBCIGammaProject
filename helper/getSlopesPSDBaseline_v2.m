...
%plotSlopes(slopes,computeSlopesAtFreqs,'black',[]);
end
...
function plotSlopes(slopes,computeSlopesAtFreqs,colorNames,freqLim)
...
%         seSlopeVals(i,j) = getSEMedian(xData{i,j}(:,2),1000);
    ...
    plot(computeSlopesAtFreqs,mSlopeVals(i,:),'color',colorNames(i,:),'LineWidth',2);
    hold('on');
    plot(computeSlopesAtFreqs,mSlopeVals(i,:)+seSlopeVals(i,:),'color',colorNames(i,:),'linestyle','--');
    plot(computeSlopesAtFreqs,mSlopeVals(i,:)-seSlopeVals(i,:),'color',colorNames(i,:),'linestyle','--');
end
...
%     showSignificance(computeSlopesAtFreqs,sigPoints,'g',yLim(1),0.05);
% end
sigPointsBonferroni = find(pValues<0.05/length(computeSlopesAtFreqs));
if ~isempty(sigPointsBonferroni)
    showSignificance(computeSlopesAtFreqs,sigPointsBonferroni,'k',yLim(1),0.05);
end
end
...
function showSignificance(xVals,sigPoints,colorName,yVal,dY)
...
if ~isempty(sigPoints)
    ...
    for i=1:length(sigPoints)
        ...
        patch(patchLocX,patchLocY,colorName,'Parent','EdgeColor',colorName);
    end
end
end